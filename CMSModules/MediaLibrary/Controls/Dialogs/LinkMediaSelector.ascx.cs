...
public partial class CMSModules_MediaLibrary_Controls_Dialogs_LinkMediaSelector : LinkMediaSelector
{
    ...
    protected override void OnPreRender(EventArgs e)
    {
        ...
        if (LibraryInfo == null)
        {
            DisplayInfo(GetString("dialogs.libraries.nolibrary"));
    ...
    private string GetParentFullPath(string path, bool includeRoot)
    {
        if (!string.IsNullOrEmpty(path))
        {
            ...
            path = lastSlash > -1 ? path.Substring(0, lastSlash).Trim('/') : "";
    ...
    private void IncludeFolders(DataSet ds, string searchText)
    {
        ...
        if ((ds != null) && (ds.Tables[0] != null))
        {
            ...
            string dirPath = DirectoryHelper.CombinePath(MediaLibraryRootFolder, LibraryInfo.LibraryFolder) + ((StartingPath != "") ? "\\" + StartingPath : "") + ((LastFolderPath != "") ? "\\" + LastFolderPath : "");
    ...
    private void LoadUserConfiguration()
    {
        if (CMSContext.CurrentUser.UserSettings.UserDialogsConfiguration != null)
        {
            ...
            string libraryName = (dialogConfig.ContainsColumn("media.libraryname") ? (string)dialogConfig["media.libraryname"] : "");
            string path = (dialogConfig.ContainsColumn("media.path") ? (string)dialogConfig["media.path"] : "");
            string siteName = (dialogConfig.ContainsColumn("media.sitename") ? (string)dialogConfig["media.sitename"] : "");

            // Set user dialogs configuration only if all sites avaliable in selector or selected site is equal to users
    ...
    private void InitializeControls()
    {
        ...
        if (dialogConfig != null)
        {
            ...
            string viewMode = (dialogConfig.ContainsColumn("media.viewmode") ? (string)dialogConfig["media.viewmode"] : "");
            if (viewMode != "")
            {
    ...
    private void SelectSite()
    {
        ...
        else
        {
            ...
            if (CMSContext.CurrentUser.UserSettings.UserDialogsConfiguration != null)
            {
                ...
                // Get site name
                string usersSiteName = (dialogConfig.ContainsColumn("media.sitename") ? (string)dialogConfig["media.sitename"] : "");
                if (usersSiteName != "")
                {
...
function imageEdit_Refresh(guid){{
    ...
    private void LoadDataSource(string searchText)
    {
        ...
        if ((FolderPath != null) && (LibraryID > 0))
        {
            ...
            if (err == "")
            {
                ...
                string where = String.Format("FilePath LIKE N'{0}%' {{escape '{1}'}} AND FilePath NOT LIKE N'{2}_%/%' {{escape '{1}'}} AND FileLibraryID = {3}", (String.IsNullOrEmpty(filePath) ? "" : filePath + "/"), PERCENT_ESC_CHAR, filePath, LibraryID);
    ...
    private void HandleFolderAction(string folderPath, bool isNewFolder, bool selectAndDisplay)
    {
        ...
        if (!folderTree.FolderExists(folderToCheck))
        {
            ...
            FolderPath = "";
            selectPath = StartingPath == "" ? (LibraryInfo == null) ? "" : LibraryInfo.LibraryFolder : folderTree.MediaLibraryFolder;
        }
    ...
    protected void hdnButton_Click(object sender, EventArgs e)
    {
        ...
        switch (CurrentAction)
        {
            ...
            case "morefolderselect":
                ...
                folderPath = StartingPath != "" ? StartingPath + folderPath : GetFullFilePath(folderPath);
            ...
            case "parentselect":
                string path = StartingPath != "" ? GetParentFullPath(StartingPath + LastFolderPath, false) : GetParentFullPath(LastFolderPath);
            ...
            case "closelisting":
                ...
                folderPath = StartingPath != "" ? StartingPath + LastFolderPath : GetFullFilePath(LastFolderPath);
    ...
    protected void librarySelector_LibraryChanged(object sender, EventArgs e)
    {
        ...
        if (LibraryInfo != null)
        {
            ...
            string fullPath = !URLHelper.IsPostback() ? ValidationHelper.GetString(RequestStockHelper.GetItem("FolderPath"), "") : GetCompletePath(FolderPath);
