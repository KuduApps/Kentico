...
public partial class CMSModules_Forums_Filters_ForumGroupFilter : CMSAbstractBaseFilterControl
{
    ...
    protected override void OnInit(EventArgs e)
    {
        ...
        filteredControl = this.FilteredControl as CMSUserControl;
        ...
        if (!RequestHelper.IsPostBack())
        {
            if (siteId > 0)
            {
                ...
                if (!DataHelper.DataSourceIsEmpty(defaultGroup))
                {
                    ...
                    if (defaultGroupId > 0)
                    {
                        this.forumGroupSelector.UniSelector.Value = defaultGroupId;
                        this.WhereCondition = GenerateWhereCondition(defaultGroupId);
                    }
                };
            }
        }
