...
public partial class CMSModules_Forums_Controls_Layouts_Tree_Threads : ForumViewer, IPostBackEventHandler
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        postTreeElem.SelectOnlyApproved = !((fi != null) && this.EnableOnSiteManagement && fi.ForumModerated && ForumContext.UserIsModerator(ForumContext.CurrentForum.ForumID, this.CommunityGroupID));
        ...
        if (selected > 0)
        {
            ...
            if (fpi != null)
            {
                ...
                ucAbuse.ReportTitle = ResHelper.GetString("Forums_WebInterface_ForumPost.AbuseReport", CMS.SettingsProvider.SettingsKeyProvider.GetStringValue(CMSContext.CurrentSiteName + ".CMSDefaultCulture")) + fpi.PostText;
                ucAbuse.CMSPanel.CommunityGroupID = this.CommunityGroupID;
                ucAbuse.CMSPanel.SecurityAccess = this.AbuseReportAccess;
                ucAbuse.CMSPanel.Roles = this.AbuseReportRoles;
            }
        }
    }
    ...
    {
        this.plcPostBody.Visible = false;
    }
