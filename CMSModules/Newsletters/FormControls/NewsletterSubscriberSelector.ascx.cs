...
public partial class CMSModules_Newsletters_FormControls_NewsletterSubscriberSelector : FormEngineUserControl
{
    ...
    public Button DialogButton
    {
        ...
        {
            EnsureChildControls();
            return this.usSubscribers.ButtonDialog;
        }
    }
    ...
    public Image ImageDialog
    {
        ...
        {
            EnsureChildControls();
            return this.usSubscribers.ImageDialog;
        }
    }
    ...
    public HyperLink LinkDialog
    {
        ...
        {
            EnsureChildControls();
            return this.usSubscribers.LinkDialog;
        }
    }
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (string.IsNullOrEmpty(this.ResourcePrefix))
        {
            this.usSubscribers.ResourcePrefix = "selectsubscriber";
        }

        // Add sites filter
        if (ShowSiteFilter)
        {
            this.usSubscribers.FilterControl = "~/CMSFormControls/Filters/SiteFilter.ascx";

            this.usSubscribers.SetValue("DefaultFilterValue", (this.SiteID > 0) ? this.SiteID : CMSContext.CurrentSiteID);
            this.usSubscribers.SetValue("FilterMode", "subscriber");
        }

        // Select users depending on site; if filter enabled, where condition is added from filter itself
        if (!ShowSiteFilter && (this.SiteID >= 0))
        {
