...
public partial class CMSModules_ProjectManagement_Controls_UI_ProjectTask_List : CMSAdminListControl, IPostBackEventHandler, INamingContainer
{
    ...
    public string EditPageURL
    {
        get
        {
            return ValidationHelper.GetString(this.GetValue("EditPageURL"), "Edit.aspx");
        }
        set
        {
            this.SetValue("EditPageURL", value);
        }
    }
    ...
    public string EditPageParameters
    {
        get
        {
            return ValidationHelper.GetString(this.GetValue("EditPageParameters"), String.Empty);
        }
        set
        {
            this.SetValue("EditPageParameters", value);
        }
    }
    ...
    public bool IgnoreCommunityGroup
    {
        get
        {
            return ValidationHelper.GetBoolean(this.GetValue("IgnoreCommunityGroup"), false);
        }
        set
        {
            this.SetValue("IgnoreCommunityGroup", value);
        }
    }
    ...
    protected override void OnPreRender(EventArgs e)
    {
        ...
        this.lblReminderError.Visible = !string.IsNullOrEmpty(this.lblReminderError.Text);
            ...
            ScriptHelper.RegisterClientScriptBlock(this, typeof(string), "EditRedirectTaskScript", ScriptHelper.GetScript(@" 
                function EditProjectTask(id) {
                    ...
                    if (usePostback == 'False') {
                        window.location.replace('" + this.EditPageURL + @"?projectTaskId=' + id + '&projectid='+projectId+'&groupid='+groupId+'" + EditPageParameters + @"' );
                        return false;
                    }
                    return true;
                }    
    ...
    {
        // Check whethe rgroups should be checked
        if (!IgnoreCommunityGroup)
        {
            ...
            {
                gridElem.WhereCondition = SqlHelperClass.AddWhereCondition(gridElem.WhereCondition, "ProjectGroupID = " + this.CommunityGroupID);
            }
            else
            {
