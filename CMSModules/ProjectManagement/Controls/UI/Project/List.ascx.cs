...
public partial class CMSModules_ProjectManagement_Controls_UI_Project_List : CMSAdminListControl, IPostBackEventHandler
{
    ...
    /// <summary>
    /// Gest or sets the target for project link.
    /// </summary>
    ...
    public string EditPageURL
    {
        get
        {
            ...
            if (this.CommunityGroupID > 0 && !this.IsLiveSite)
            {
                url = URLHelper.UpdateParameterInUrl(url, "groupid", this.CommunityGroupID.ToString());
                url += "&";
            }
            else
            {
    ...
    public int PageSize
    {
        get
        {
            return ValidationHelper.GetInteger(this.GetValue("PageSize"), 25);
        }
        set
        {
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        //Project list webpart condition - ussually used in project list
        if (ProjectNodeID != 0)
        {
