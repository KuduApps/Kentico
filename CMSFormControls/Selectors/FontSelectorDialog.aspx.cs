...
public partial class CMSFormControls_Selectors_FontSelectorDialog : CMSModalPage
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!RequestHelper.IsPostBack())
        {
            ...
            //setup font style
            if (txtFontStyle.Text.ToLower().Contains("bold"))
            {
                lblSampleText.Font.Bold = true;
            }
            if (txtFontStyle.Text.ToLower().Contains("italic"))
            {
    ...
    private void FillFontTypeListBox()
    {
        foreach (FontFamily fontName in FontFamily.Families)
        {
            ...
            if (txtFontType.Text.ToLower() == fontName.Name.ToLower())
            {
                li.Selected = true;
            }
    ...
    protected void btnOk_Click(object sender, EventArgs e)
    {
        if (txtFontSize.Text != String.Empty && txtFontStyle.Text != String.Empty && txtFontType.Text != String.Empty)
        {
            ...
            switch (lstFontStyle.SelectedValue.ToLower())
            {
                case "bold":
                    fs = FontStyle.Bold;
                    break;
            ...
            string submitScript = ScriptHelper.GetScript(String.Format("wopener.getParameters('{0}','{1}','{2}'); window.close();", ret, mHiddenFieldID, mFontTypeID));
            ScriptHelper.RegisterClientScriptBlock(this.Page, typeof(Page), "SubmitScript", submitScript);
        }
    }
