...
public partial class CMSFormControls_Selectors_GridColumnDesigner : DesignerPage
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (string.IsNullOrEmpty(QueryHelper.GetString("queryname", string.Empty)) || QueryHelper.ValidateHash("hash"))
        {
            ...
            if ((!RequestHelper.IsPostBack()) || (FirtsLoad < 2))
            {
                ...
                switch (mTypeOfInput)
                {
                    case 1: LoadFromDataClass(); break;

                    case 2: LoadFromQuery(); break;
                    default: break;
                }
                ...
                script.AppendLine("document.getElementById('" + hdnClassNames.ClientID + "').value = wopener.GetClassNames('" + mColId + "'); ");
                script.AppendLine("document.getElementById('" + hdnSelectedColumns.ClientID + "').value = wopener.GetSelectedColumns('" + mSelColId + "');");
                script.AppendLine("__doPostBack('" + ItemSelection1.RightColumListBox.ClientID + @"',''); ");
                ltlLoad.Text = ScriptHelper.GetScript(script.ToString());
    ...
    /// </summary>
    void ItemSelection1_OnPostBack(object sender, EventArgs e)
    {
    ...
    protected void CreateXML()
    {
        ...
        {
            hdnSelectedColumns.Value = "<columns>";
            ...
            for (int i = 0; i < ItemSelection1.RightColumListBox.Items.Count; i++)
            {
                ...
                hdnSelectedColumns.Value += "<column name=\"" + ItemSelection1.RightColumListBox.Items[i].Text + "\" ";

                if (mIsChanged)
                    hdnSelectedColumns.Value += "header=\"" + mChanged[0, 2] + "\" ";
                else
                {
                    hdnSelectedColumns.Value += "header=\"\" ";
                }

                hdnSelectedColumns.Value += "type=\"";

                if ((mIsChanged) && (mChanged[0, 3] != null && mChanged[0, 3] == "link"))
                    hdnSelectedColumns.Value += "link";
                else
                {
                    hdnSelectedColumns.Value += "bound";
                }

                hdnSelectedColumns.Value += "\">";
                hdnSelectedColumns.Value += "</column>";
            }
