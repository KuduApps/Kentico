...
function SetVariant(itemCode, variantId) {
    ...
    if (itemInfo != null) {
        ...
        if (hdnObj != null) {
            ...
            var sliderPosition = itemInfo[0].indexOf(variantId) + 1;
...
function OnSetContextMenuParameter(menuId, params) {
    if ((menuId == "webPartMenu") || (menuId == "webPartZoneMenu")) {
        ...
        var isWebPartMenu = (params.length > 3);
        var showVariantsDenied = false;

        // Check if variants menu is allowed
        if (isWebPartMenu && (zonesWithVariantsDenied.indexOf(zoneId) >= 0)) {
            displayMVTWebPartMenu = false;
            displayCPWebPartMenu = false;
        } else if (!isWebPartMenu && (zonesWithVariantsDenied.indexOf('#' + zoneId) >= 0)) {
