...
(function($, undefined) {
    ...
    $.extend(Datepicker.prototype, {
        ...
        _showDatepicker: function(target) {
            $.datepicker._showDatepicker($(target)[0]);
        },

        _setLocationDatepicker: function(target, id) {
            var position = $j("#" + id).offset();
            $.datepicker._pos = [position.left, position.top + 20];
        },
        ...
        _checkExternalClick: function(event) {
            ...
            var iconID = $.datepicker._curInst.settings.IconID;

            if ($target[0].id != $.datepicker._mainDivId && $target[0].id != iconID &&
	   ...
	   @param  format    string - the desired format of the date
        ...
        _generateHTML: function(inst) {
			...
			'.datepicker._saveAndClose(\'#' + inst.id + '\');' + okCommand + '";">' + this._get(inst, 'closeText') + '</button>' : '');
            ...
            var NAButton = displayNA ? '<button type="button" ' + buttonRestStyle + ' class="datetime-ui-datepicker-current datetime-ui-state-default datetime-ui-priority-secondary datetime-ui-corner-all" onclick="DP_jQuery_' + dpuuid +
            	'.datepicker._clearDate(\'#' + inst.id + '\');' + okCommand + '">' + this._get(inst, 'NAText') + '</button>' : '';
            ...
            for (var row = 0; row < numMonths[0]; row++) {
                ...
                for (var col = 0; col < numMonths[1]; col++) {
                    ...
                    if (showTimePanel) {
                        ...
                        var seconds = "<td>:</td><td><input class='datetime-ui-time-textbox' type='text' onKeyDown='return DP_jQuery_" + dpuuid + ".datepicker._checkHours(event,id,59,0);' id='" + inst.id + "_second' value = '" + this._addZero(dt.getSeconds(), zeroSecond) + "'></td>" +
                                   "<td><div class='datetime-ui-time-arrow_move datetime-ui-icon datetime-ui-timeicon-n' onclick=\"DP_jQuery_" + dpuuid + ".datepicker._moveTime ('" + inst.id + "_second',59,0,5," + zeroSecond + ",null);\"></div><div class='datetime-ui-time-arrow_move datetime-ui-icon datetime-ui-timeicon-s' onclick=\"DP_jQuery_" + dpuuid + ".datepicker._moveTime ('" + inst.id + "_second',59,0,-5," + zeroSecond + ",null);\"></div></td>";

                        calender += "<div class=\"RightAlign\">" +
                                   "<table><tr>" + (isRTL ? "" : "<td style=\"width:100%\"/>") + "<td><input class='datetime-ui-time-textbox' type='text' onKeyDown='return DP_jQuery_" + dpuuid + ".datepicker._checkHours(event,id," + hourMax + "," + hourMin + ");' id='" + inst.id + "_hour' value = '" + this._addZero(hours, zeroHour) + "'></td>" +
                                   "<td><div class='datetime-ui-time-arrow_move datetime-ui-icon datetime-ui-timeicon-n' onclick=\"DP_jQuery_" + dpuuid + ".datepicker._moveTime ('" + inst.id + "_hour'," + hourMax + "," + hourMin + ",1," + zeroHour + "," + hourShiftID + ");\" ></div><div class='datetime-ui-time-arrow_move datetime-ui-icon datetime-ui-timeicon-s' onclick=\"DP_jQuery_" + dpuuid + ".datepicker._moveTime ('" + inst.id + "_hour'," + hourMax + "," + hourMin + ",-1," + zeroHour + "," + hourShiftID + ");\"></div></td><td>:</td>" +
                                   "<td><input class='datetime-ui-time-textbox' type='text' onKeyDown='return DP_jQuery_" + dpuuid + ".datepicker._checkHours(event,id,59,0);' id='" + inst.id + "_minute' value ='" + this._addZero(dt.getMinutes(), zeroMinute) + "'></td>" +
                                   "<td><div class='datetime-ui-time-arrow_move datetime-ui-icon datetime-ui-timeicon-n' onclick=\"DP_jQuery_" + dpuuid + ".datepicker._moveTime ('" + inst.id + "_minute',59,0,5," + zeroMinute + ",null);\" ></div><div class='datetime-ui-time-arrow_move datetime-ui-icon datetime-ui-timeicon-s' onclick=\"DP_jQuery_" + dpuuid + ".datepicker._moveTime ('" + inst.id + "_minute',59,0,-5," + zeroMinute + ",null);\" ></div></td>" +
