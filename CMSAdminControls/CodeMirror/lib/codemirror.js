...
var CodeMirror = (function() {
    ...
    function CodeMirror(place, givenOptions) {
      ...
      '<div class="CodeMirror-scroll cm-s-' + options.theme + '" ' + (options["dynamicHeight"] ? '' : ' style="height:' + options["height"] + ';"') + '>' +
        '<div style="position: ' + (options["dynamicHeight"] ? 'relative' : 'absolute') + '; z-index: 9999;">' + // Set to the height of the text, causes scrolling
    ...
    CodeMirror.fromTextArea = function(textarea, options) {
        ...
        /* CMS */
        instance.savePosition = function(marker) {
            var e = instance;
            var cursor = e.getCursor();
            document.getElementById(marker).value = e.getScrollerElement().scrollTop + ";" + "true" + ";" + cursor.line + ";" + cursor.ch;
        }
...
function Toolbar(editor) {
    ...
    // Indent all is disabled in RTL
    var indentDisabled = readOnly || editor.getOption("direction") == "rtl";
    //makeButton(getLocalizedString("IndentAll"), "indentAll", indentDisabled);
    ...
    if (false && editor.getOption("lineCounting") && !editor.getOption("readOnly")) {
    ...
    function jumpToLine(e) {
        e = e || window.event;
        if (e.keyCode == 13) {
            var totalLines = editor.lineNumber(editor.lastLine()) - 1;
            var line = Number(currentLineBox.value);
            if (!isNaN(line)) {
                line = (line < totalLines) ? line : totalLines;
                line = (line < 1) ? 1 : line;
                self.jump(line);
                currentLineBox.value = line;
            }
            return false;
        }
    }
    ...
    function updateLineNumbers() {
        ...
        var totalLines = editor.lineNumber(editor.lastLine());
        if ((editor.win.gecko && editor.win.document.getElementsByTagName("br").length > 0) ||
            (editor.win.webkit && editor.editor.container.lastChild.hackBR))
            totalLines--;
...
function StaticBookmarks(editor, bookmarks) {
    ...
    if (editor.getOption("direction") != "rtl") {
        scroller.style.marginRight = bookmarksPanel.clientWidth + "px";
    }
    else {
        scroller.style.marginLeft = bookmarksPanel.clientWidth + "px";
    }
...
function DynamicBookmarks(editor, bookmarkStyle) {
    ...
    // make space in the editor for bookmarks panel (check for flow direction)
    if (editor.getOption("direction") != "rtl") {
        wrapper.style.marginRight = bookmarksPanel.clientWidth + "px";
    }
    else {
        wrapper.style.marginLeft = bookmarksPanel.clientWidth + "px";
    }
...
DynamicBookmarks.prototype = {
    ...
    show: function() {
        if (!this.visible) {
            ...
            if (this.editor.toolbar != null)
                this.editor.toolbar.toolbarPanel.style.right = rightOffset + "px";
            this.editor.getWrapperElement.style.marginRight = rightOffset + "px";
            this.visible = true;
        }
    },
    ...
    hide: function() {
        if (this.visible) {
            ...
            if (this.editor.toolbar != null)
                this.editor.toolbar.toolbarPanel.style.right = "0px";
            this.editor.getWrapperElement.style.marginRight = "0";
            this.visible = false;
        }
    },
...
function Popup(editor) {
    ...
    function createSearchWindow() {
        var wnd = createWindow(this.dialogPath + "Search.aspx?editorName=" + this.editor.name, "Search", 500, 350, 'resizable=no');
    }

    function createReplaceWindow() {
        var wnd = createWindow(this.dialogPath + "Replace.aspx?editorName=" + this.editor.name, "Replace", 500, 350, 'resizable=no');
    }
