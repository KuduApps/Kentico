...
public partial class CMSWebParts_Maps_Documents_BingMaps : CMSAbstractWebPart
{
    ...
    public bool HideControlForZeroRows
    {
        get
        {
            return ValidationHelper.GetBoolean(this.GetValue("HideControlForZeroRows"), this.ucBingMap.HideControlForZeroRows);
        }
        set
        {
            this.SetValue("HideControlForZeroRows", value);
            this.ucBingMap.HideControlForZeroRows = value;
        }
    }
    ...
    public string ZeroRowsText
    {
        get
        {
            return DataHelper.GetNotEmpty(ValidationHelper.GetString(this.GetValue("ZeroRowsText"), this.ucBingMap.ZeroRowsText), this.ucBingMap.ZeroRowsText);
        }
        set
        {
            this.SetValue("ZeroRowsText", value);
            this.ucBingMap.ZeroRowsText = value;
        }
    }
    ...
    protected void SetupControl()
    {
        ...
        else
        {
            ...
            {
                this.ucBingMap.CacheItemName = this.ucDocumentSource.CacheItemName;

                if (this.ucDocumentSource.CacheMinutes <= 0)
                {
                    // If zero or less, get from the site settings
                    this.ucBingMap.CacheMinutes = SettingsKeyProvider.GetIntValue(CurrentSiteName + ".CMSCacheMinutes");
                }
                else
                {
                    this.ucBingMap.CacheMinutes = this.ucDocumentSource.CacheMinutes;
                }

                // Cache depends on data source and properties of web part
                this.ucBingMap.CacheDependencies = CacheHelper.GetCacheDependencies(this.CacheDependencies, this.ucDocumentSource.GetDefaultCacheDependendencies());
            }
            ...
            ucBingMap.HideControlForZeroRows = this.HideControlForZeroRows;

            if (!String.IsNullOrEmpty(this.ZeroRowsText))
            {
                ucBingMap.ZeroRowsText = this.ZeroRowsText;
            }
