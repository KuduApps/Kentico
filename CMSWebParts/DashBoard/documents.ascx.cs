...
public partial class CMSWebParts_DashBoard_Documents : CMSAbstractWebPart
{
    ...
    public ListingTypeEnum ListingType
    {
        get
        {
            string listingType = ValidationHelper.GetString(GetValue("ListingType"), String.Empty).ToLower();
            switch (listingType)
            {
                case "recent":
    ...
    private void SetDocumentFilter()
    {
        ...
        ucDocuments.IsLiveSite = (ViewMode == CMS.PortalEngine.ViewModeEnum.LiveSite);
    }
